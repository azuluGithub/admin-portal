...
import './CourseUpdate.style.scss';

class CourseUpdateComponent extends PureComponent {
  ...
  setData() {
    ...
    if (course) {
      const { imgUrl, title, content, category, author, date } = course;
    ...
    return (
      <div className='p-10 bg-white'>
        <form onSubmit={() => this.handleFormSubmit()}>
          <h2 className='text-xl text-bold mb-4 uppercase'>{'Update'}</h2>
          { this.renderImage() }
          { this.renderTitle() }
  ...
  render() {
    return this.renderCourseUpdate();
  }
}

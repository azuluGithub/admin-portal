...
import './StudentUpdate.style.scss';

class StudentUpdateComponent extends PureComponent {
  ...
  setData() {
    ...
    if (student) {
      const { imgUrl, title, content, category, author, date } = student;
  ...
  renderImage() {
    ...
    return (
      <div className='relative mb-4'>
        ...
        <label className="absolute bottom-1 right-1" htmlFor="student_img_upload">
    ...
    return (
      <div className='p-10 bg-white'>
        <form onSubmit={() => this.handleFormSubmit()}>
          <h2 className='text-xl text-bold mb-4 uppercase'>{'Update Student'}</h2>
          { this.renderImage() }
          { this.renderTitle() }
  ...
  render() {
    return this.renderStudentUpdate();
  }
}

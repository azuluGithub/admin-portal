...
import './LecturerUpdate.style.scss';

class LecturerUpdateComponent extends PureComponent {
  ...
  setData() {
    ...
    if (lecturer) {
      const { imgUrl, title, content, category, author, date } = lecturer;
      ...
      <div className='relative mb-4'>
        {
          !imgUrl.length ?
            ...
            <img
              ...
              src={imgUrl} 
              alt='lecturer display'
            />
        }
        <label className="absolute bottom-1 right-1" htmlFor="lecturer_img_upload">
  ...
  renderLecturerUpdate() {
    const { currentColor } = this.props;

    return (
      <div className='p-10 bg-white'>
        <form onSubmit={() => this.handleFormSubmit()}>
          <h2 className='text-xl text-bold mb-4 uppercase'>{'Update Lecturer'}</h2>
          { this.renderImage() }
          { this.renderTitle() }
  ...
  render() {
    return this.renderLecturerUpdate();
  }
}

export default LecturerUpdateComponent;